// ASTRA HFC3 research derivative.
// Upstream: n30dyn4m1c/gold-pro-scalper / XAU_Quant_Reversion_TickRobust.mq5
// Research changes: XAUUSDm default, fixed 0.01 lot, news/daily-risk overlays OFF.
// Alpha/entry/exit semantics otherwise retained from upstream source.
// Intended for MT5 Strategy Tester, Every tick based on real ticks.
//
//+------------------------------------------------------------------+
...
#property description "ASTRA HFC3 TickRobust Research - fixed 0.01 lot Strategy Tester derivative"
...
//--- Inputs: Strategy
input string   InpTradeSymbol    = "XAUUSDm";   // Trade symbol (GOLD, XAUUSD, etc.)
string         TradeSymbol;
...
//--- Inputs: Risk
input bool     InpUseDynamicRisk = false;     // Enable equity-based risk tiers (overrides InpRiskPct)
input double   InpRiskPct        = 0.10;      // Risk % per trade (used when dynamic risk is off)
input double   InpFixedLot       = 0.01;     // Research lot: fixed 0.01
input int      InpSlippage       = 30;       // Max slippage in points

//--- Inputs: News Filter (red folder / CALENDAR_IMPORTANCE_HIGH only)
input bool     InpUseNewsFilter   = false;    // Enable news time filter
...
input bool     InpUseDailyLossLimit = false;  // Enable max daily loss stop
input double   InpMaxDailyLossPct   = 99.0;  // Max daily loss % of balance (when dynamic risk is off)
...
void ExecuteTrade(ENUM_ORDER_TYPE type, double price, double meanPrice, double atrVal, double zScore) {
   ...
   // ASTRA research override: alpha evaluation must not be contaminated by
   // equity/dynamic-risk sizing. Always use the same executable broker lot.
   double lot = NormalizeLot(InpFixedLot);
   double minLot = SymbolInfoDouble(TradeSymbol, SYMBOL_VOLUME_MIN);
   if(lot < minLot || lot <= 0.0) {
      Print("Invalid fixed research lot: ", DoubleToString(lot, 2));
      return;
   }
