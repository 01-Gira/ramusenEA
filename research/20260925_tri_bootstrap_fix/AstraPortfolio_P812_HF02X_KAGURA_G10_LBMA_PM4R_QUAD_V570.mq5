...
input int    LB_InpStartCaptureWindowSec=10;
...
void LB_CsvSummary()
{
 ...
 FileWrite(LB_fh_summary,"server_utc_offset_minutes",LB_InpServerUTCOffsetMinutes);
...
void LB_EvaluateEvent(const MqlTick &q,datetime event_start)
{
 ...
 double end_mid=(q.bid+q.ask)*0.5;if(LB_start_mid<=0||end_mid<=0)return;
 double bps=(end_mid/LB_start_mid-1.0)*10000.0;
 if(MathAbs(bps)+1e-12<LB_InpImpulseThresholdBps){LB_CsvEvent("NO_TRIGGER",StringFormat("day=%d bps=%.6f",LB_day_key,bps));return;}
 ...
 if(ok){LB_cnt_open++;LB_CsvEvent("OPEN_REQUEST_OK",StringFormat("day=%d side=%d bps=%.6f entry=%.3f sl=%.3f risk=%.2f bal=%.2f risk_pct=%.4f",LB_day_key,side,bps,LB_trade.ResultPrice(),sl,risk,balance,risk_pct));}
...
int LB_OnInit()
{
 ...
 if(MathAbs(LB_InpFixedLot-0.01)>1e-12||LB_InpImpulseThresholdBps<=0||LB_InpObserveMinutes<=0||LB_InpSLDistance<=0||LB_InpMaxHoldMinutes<=0||LB_InpRiskCapPct<=0){Print("[LBMA][FATAL] invalid frozen inputs");return INIT_FAILED;}
 LB_trade.SetExpertMagicNumber(LB_InpMagic);LB_trade.SetTypeFillingBySymbol(_Symbol);LB_trade.SetDeviationInPoints(50);
...
void LB_OnTick()
{
 ...
 datetime event_start=LB_EventStartServer(now);long event_start_msc=(long)event_start*1000;
 if(!LB_start_captured)
   {
    ...
    if(q.time_msc>event_start_msc+(long)LB_InpStartCaptureWindowSec*1000)
      {LB_day_consumed=true;LB_cnt_missed_start++;LB_CsvEvent("MISSED_EVENT_START",StringFormat("day=%d first_msc=%I64d expected=%I64d",dk,q.time_msc,event_start_msc));return;}
    LB_start_captured=true;LB_start_mid=(q.bid+q.ask)*0.5;LB_start_msc=q.time_msc;
    LB_CsvEvent("START_CAPTURE",StringFormat("day=%d event=%s actual_msc=%I64d mid=%.6f dst=%s",dk,LB_TS(event_start),q.time_msc,LB_start_mid,(LB_LondonDSTFromUTC(LB_UTCFromServer(event_start))?"YES":"NO")));
   }
...
void OnDeinit(const int reason)
  {
   ...
   if(g_pf_summary!=INVALID_HANDLE)
     {
      FileWrite(g_pf_summary,"build","P812_HF02X_KAGURA_G10_LBMA_PM4R_QUAD_V570_20260925");
