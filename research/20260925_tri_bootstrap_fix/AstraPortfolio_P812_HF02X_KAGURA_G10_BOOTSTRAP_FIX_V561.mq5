...
#property strict
#property description "ASTRA Portfolio: frozen P812 + HF02X + KAGURA G10; deterministic bootstrap parity fix; no alpha tuning."
#property description "P812 frozen + HF02 breakout-retest rescue sleeve; fixed 0.01, hard SL/TP, no martingale/grid."
...
input ENUM_ASTRA_TRI_MODE InpPortfolioMode=TRI_P812_HF02X_KAGURA_G10;

bool PortfolioPrimaryActive()
  { return(InpPortfolioMode!=TRI_KAGURA_ONLY); }
bool PortfolioHF02XActive()
  { return(InpPortfolioMode==TRI_P812_HF02X || InpPortfolioMode==TRI_P812_HF02X_KAGURA_G10); }
bool PortfolioKaguraActive()
  { return(InpPortfolioMode==TRI_KAGURA_ONLY || InpPortfolioMode==TRI_P812_KAGURA_G10 || InpPortfolioMode==TRI_P812_HF02X_KAGURA_G10); }
...
int OnInit()
  {
   ...
   if(InpWriteAuditCommon)
     {
      g_pf_summary=FileOpen("ASTRA_P812_HF02X_KAGURA_PORTFOLIO_SUMMARY.csv",FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_COMMON,',');
      if(g_pf_summary!=INVALID_HANDLE) FileWrite(g_pf_summary,"metric","value");
     }

   PrintFormat("[TRI][READY] mode=%d P812=%s HF02X=%s KAGURA=%s G10=%s cap=%.2f%%",
               (int)InpPortfolioMode,(PortfolioPrimaryActive()?"ON":"OFF"),(PortfolioHF02XActive()?"ON":"OFF"),
               (PortfolioKaguraActive()?"ON":"OFF"),(K_InpG10Enabled?"ON":"OFF"),K_InpG10RiskCapPct);
   Print("[TRI][FREEZE] P812 families 1..11 frozen; HF02X family 0 frozen; KAGURA rules + G10 frozen. No alpha tuning in this build.");
