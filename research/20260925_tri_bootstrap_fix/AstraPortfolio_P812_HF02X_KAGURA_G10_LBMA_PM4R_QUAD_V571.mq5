...
void K_ForceCloseIfNeeded(datetime now)
{
 ...
 if(now<K_DayAt(now,K_InpForceCloseHour))return;
 K_trade.SetExpertMagicNumber(K_InpMagic); if(!K_trade.PositionClose(ticket) && K_InpVerbose) Print("[KAGURA][FORCE_CLOSE_FAIL] ",K_trade.ResultRetcodeDescription());
}
...
void K_EvaluateClosedM5(datetime now)
{
 ...
 {
  if(dk<K_InpFrozenStartDay || dk>K_InpFrozenEndDay || dk==20260731) return;
 }
...
input int    LB_InpBoundaryLookbackSec=300; // exact raw-parity quote: last known tick at/before each boundary
...
input bool   LB_InpFrozenParityPopulationGuard=true; // historical raw research coverage only; OFF prospective/live.
...
bool LB_CsvInit()
{
 ...
 FileWrite(LB_fh_signals,"time","day","event_start","event_end","london_dst","start_mid","end_mid","impulse_bps","side","spread","entry_quote","sl","risk_usd","balance","risk_pct","decision","primary_open","kagura_open");
...
void LB_CsvSummary()
{
 ...
 FileWrite(LB_fh_summary,"server_utc_offset_minutes",LB_InpServerUTCOffsetMinutes);FileWrite(LB_fh_summary,"boundary_mode","LAST_VALID_TICK_AT_OR_BEFORE");FileWrite(LB_fh_summary,"boundary_lookback_sec",LB_InpBoundaryLookbackSec);
 FileWrite(LB_fh_summary,"frozen_parity_population_guard",LB_InpFrozenParityPopulationGuard);
...
void LB_ResetDay(int dk){LB_day_key=dk;LB_start_captured=false;LB_day_consumed=false;LB_start_mid=0.0;LB_start_msc=0;}

bool LB_QuoteAtOrBefore(const long target_msc,MqlTick &out)
{
 if(target_msc<=0)return false;
 ulong to=(ulong)target_msc;
 ulong span=(ulong)MathMax(1,LB_InpBoundaryLookbackSec)*1000;
 ulong from=(to>span?to-span:0);
 MqlTick tt[];ArraySetAsSeries(tt,false);
 ResetLastError();
 int n=CopyTicksRange(_Symbol,tt,COPY_TICKS_ALL,from,to);
 if(n<=0){if(LB_InpVerbose)PrintFormat("[LBMA][BOUNDARY_MISS] target=%I64d err=%d",target_msc,GetLastError());return false;}
 for(int i=n-1;i>=0;i--)
   {
    if((long)tt[i].time_msc>target_msc)continue;
    if(tt[i].bid<=0.0||tt[i].ask<=0.0||tt[i].ask<tt[i].bid)continue;
    out=tt[i];return true;
   }
 return false;
}
...
void LB_EvaluateEvent(const MqlTick &q,datetime event_start)
{
 ...
 // Raw simulator convention: quote-as-of each event boundary, i.e. the LAST
 // valid quote whose time_msc <= boundary. This avoids using information from
 // a tick that arrived after 15:00:00.000 or after 15:04:00.000.
 MqlTick q0,q1;
 if(!LB_QuoteAtOrBefore(event_start_msc,q0)||!LB_QuoteAtOrBefore(event_end_msc,q1))
   {
    LB_day_consumed=true;LB_cnt_missed_start++;
    LB_CsvEvent("BOUNDARY_QUOTE_MISS",StringFormat("day=%d start=%I64d end=%I64d",LB_day_key,event_start_msc,event_end_msc));
    return;
   }

 LB_day_consumed=true;LB_cnt_observations++;
 LB_start_captured=true;LB_start_mid=(q0.bid+q0.ask)*0.5;LB_start_msc=(long)q0.time_msc;
 double end_mid=(q1.bid+q1.ask)*0.5;if(LB_start_mid<=0||end_mid<=0)return;
 double bps=(end_mid/LB_start_mid-1.0)*10000.0;
 LB_CsvEvent("BOUNDARY_QUOTES",StringFormat("day=%d start_target=%I64d start_tick=%I64d end_target=%I64d end_tick=%I64d start_mid=%.6f end_mid=%.6f bps=%.9f",LB_day_key,event_start_msc,(long)q0.time_msc,event_end_msc,(long)q1.time_msc,LB_start_mid,end_mid,bps));
 ...
 double risk=0,balance=AccountInfoDouble(ACCOUNT_BALANCE),risk_pct=DBL_MAX;if(LB_InitialRiskUSD(side,entry,sl,risk)&&balance>0)risk_pct=100.0*risk/balance;
 ...
 if(LB_fh_signals!=INVALID_HANDLE){FileWrite(LB_fh_signals,LB_TS(TimeCurrent()),LB_day_key,LB_TS(event_start),LB_TS((datetime)((long)event_start+LB_InpObserveMinutes*60)),LB_LondonDSTFromUTC(LB_UTCFromServer(event_start)),LB_start_mid,end_mid,bps,(side>0?"BUY":"SELL"),spread,entry,sl,risk,balance,risk_pct,decision,primary_open,kagura_open);FileFlush(LB_fh_signals);}
 if(decision!="ACCEPT")return;
 LB_trade.SetExpertMagicNumber(LB_InpMagic);LB_trade.SetTypeFillingBySymbol(_Symbol);LB_trade.SetDeviationInPoints(50);
 string c="LBMA-PM4R";ResetLastError();bool ok=(side>0?LB_trade.Buy(LB_InpFixedLot,_Symbol,0,sl,0,c):LB_trade.Sell(LB_InpFixedLot,_Symbol,0,sl,0,c));
 if(ok){LB_cnt_open++;LB_CsvEvent("OPEN_REQUEST_OK",StringFormat("day=%d side=%d bps=%.9f entry=%.3f sl=%.3f risk=%.2f bal=%.2f risk_pct=%.4f",LB_day_key,side,bps,LB_trade.ResultPrice(),sl,risk,balance,risk_pct));}
 else {LB_cnt_order_fail++;LB_CsvEvent("ORDER_FAIL",StringFormat("day=%d side=%d ret=%u %s",LB_day_key,side,LB_trade.ResultRetcode(),LB_trade.ResultRetcodeDescription()));}
}
...
int LB_OnInit()
{
 ...
 if(MathAbs(LB_InpFixedLot-0.01)>1e-12||LB_InpImpulseThresholdBps<=0||LB_InpObserveMinutes<=0||LB_InpSLDistance<=0||LB_InpMaxHoldMinutes<=0||LB_InpRiskCapPct<=0||LB_InpBoundaryLookbackSec<=0){Print("[LBMA][FATAL] invalid frozen inputs");return INIT_FAILED;}
 LB_trade.SetExpertMagicNumber(LB_InpMagic);LB_trade.SetTypeFillingBySymbol(_Symbol);LB_trade.SetDeviationInPoints(50);
...
void LB_OnTick()
{
 ...
 datetime now=TimeCurrent();int dk=LB_DayKey(now);if(dk!=LB_day_key)LB_ResetDay(dk);
 if(LB_InpFrozenParityPopulationGuard&&(dk<LB_InpFrozenStartDay||dk>LB_InpFrozenEndDay))return;
 if(LB_day_consumed)return;
...
void OnDeinit(const int reason)
  {
   ...
   if(g_pf_summary!=INVALID_HANDLE)
     {
      FileWrite(g_pf_summary,"build","P812_HF02X_KAGURA_G10_LBMA_PM4R_QUAD_V571_20260926");
